function [xhathist,Phist,sigmahist,enuhist] = ...
    prob9_particle_filter(zkhist, xhat0, P0, Q, R, Gamma, F, H, Ns)
...
for i = 1:n
    ...
    for j = 1:Ns
        ...
        chi(:, j) = F * chi(:, j) + Gamma * vss(:, j); % propagate to k+1

        log_wtil(j) = log(w(j)) - .5 * (z - H*chi(:, j))' * Rinv * (z - H*chi(:, j));
