...
Rk = sigmaphi^2; % measurement noise
Ra = chol(Rk);
Rainv = inv(Ra);
Hk = [1 0 0];
Ha = Rainv' * Hk;
% zahist = Rainv' * zphihist;
...
for j = 1:k
    ...
    zphij = zphihist(j,1);
    nj = round((zphij - xtilj(1)) / 2 / pi); % ANSWER
    jp1 = j + 1;
    nhist(jp1,1) = nj;
    %
    %  Measurement update.
    ...
    Tb = q'; % ANSWER
    za = Rainv' * (zphihist(j) - 2*pi*nj); % ANSWER
    temp = Tb * [zscripttilj; za]; % ANSWER
    ...
    enuhist(jp1,1) = zrscriptj' * zrscriptj; % ANSWER
end
