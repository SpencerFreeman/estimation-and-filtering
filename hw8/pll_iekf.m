...
%
Rscriptj = inv(chol(P0))'; % ANSWER
zscriptj = Rscriptj * xhat0; % ANSWER
%
...
Rk = diag([sigma^2, sigma^2]); % measurement noise
Ra = chol(Rk);
Rainv = inv(Ra);
% Hk = [1 0 0];
% Ha = Rainv' * Hk;
...
for j = 1:k
    ...
    %
    [q, r] = qr([Rvv, zeros(nv, nx); ...
        -Rscriptjm1 * FinvGamma, Rscriptjm1 * Finv]); % ANSWER
    Ta = q'; % ANSWER
    temp = Ta * [zeros(nv, 1); zscriptjm1]; % ANSWER
    Rscripttilj = r(nv + 1:end, nv + 1:end); % ANSWER
    zscripttilj = temp(nv + 1:end, :); % ANSWER
    %
    %  Measurement update.
    %
    xlinearizej = inv(Rscripttilj) * zscripttilj; % ANSWER
    for iiter = 1:Niter
        cosphij = cos(xlinearizej(1,1));
        sinphij = sin(xlinearizej(1,1));
        Aj = xlinearizej(4,1);
        Ha = Rainv' * [ ...
            -Aj * sinphij, 0, 0, cosphij; ...
             Aj * cosphij, 0, 0, sinphij]; % ANSWER

        z = zhist(j, :)';
        za = Rainv' * z; % ANSWER

        za_linearize = Rainv' * Aj * [cosphij; sinphij]; % nonlinear h()
        delza = za - za_linearize - Ha*xlinearizej; % ANSWER

        [q, r] = qr([Rscripttilj; Ha]); % ANSWER
        Tb = q'; % ANSWER
        temp = Tb * [zscripttilj; delza]; % ANSWER

        Rscriptj = r(1:nx, :); % ANSWER
        zscriptj = temp(1:nx, :); % ANSWER
        zrscriptj = temp(nx + 1:end, :); % ANSWER
    ...
    % zscriptrj = zrscriptj;
    enuhist(jp1,1) = zrscriptj' * zrscriptj;
end
